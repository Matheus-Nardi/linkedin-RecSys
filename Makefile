...
dashboard: ## Inicia o Dashboard Streamlit (http://localhost:8501)
	docker run --rm -it -p 8501:8501 -v "$$(pwd)":/app $(IMAGE_NAME) streamlit run dashboard.py --server.address=0.0.0.0 --server.port=8501

figuras: ## Executa o script de geração de figuras da EDA
	docker run --rm -it -v "$$(pwd)":/app $(IMAGE_NAME) python gerar_figuras_eda.py
