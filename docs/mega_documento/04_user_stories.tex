...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Dado que} o avaliador acessa o \textit{Modo Avaliador $\to$ Duelo dos Modelos} no painel interativo (\texttt{app/dashboard.py});
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \textbf{Então} o sistema deve discriminar a data/hora e o script exato gerador de cada artefato serializado (\texttt{metadados\_cf.pkl} e \texttt{metadados\_cbf.pkl}), comprovar a conformidade com a LGPD pelo uso de dados corporativos públicos e personas sintéticas sem qualquer PII, e declarar explicitamente a natureza \textit{self-fulfilling} da avaliação bilinear e as limitações de cobertura textual do catálogo.
\end{itemize}
...
Para assegurar o fechamento metodológico do TDSP e conectar formalmente o valor de negócio à engenharia de software desenvolvida, a Tabela~\ref{tab:matriz_rastreabilidade} sintetiza o encadeamento integral entre as 5 Histórias de Usuário, as personas centrais, os módulos de código em \texttt{src/}, os modos de navegação do dashboard e as métricas de validação empírica.
...
\toprule
\textbf{User Story} & \textbf{Persona Central} & \textbf{Fase} & \textbf{Modo no Dashboard} & \textbf{Módulo de Código} & \textbf{Métrica de Validação} \\ \midrule
\textbf{US 1 (Mercado)} & Candidato em Recolocação & 1 & Modo Candidato $\to$ EDA (Tabs H1--H5) & \texttt{src/gerar\_figuras\_eda.py} & Medianas Salariais e Salto de CTR \\
\textbf{US 2 (Cold Start)} & Candidato sem Histórico & 1 & Modo Candidato $\to$ Simulador CBF & \texttt{src/recomendador.py} & Cosseno $\ge 0{,}50$ e Re-ranking Scoped \\
\textbf{US 3 (Perfil Latente)} & Plataforma / Recrutador & 2 & Modo Candidato $\to$ Feed CF (SVD) & \texttt{src/filtragem\_colaborativa.py} & Precision@10 ($1{,}000$) e Escore Bruto \\
\textbf{US 4 (Explicabilidade)} & Candidato com Histórico & 2 & Modo Candidato $\to$ Waterfall Altair & \texttt{src/filtragem\_colaborativa.py} & Decomposição Aditiva $\mu + b_u + b_i + q_i^T p_u$ \\
\textbf{US 5 (Avaliador)} & Professora / Avaliador & 3 & Modo Avaliador $\to$ Duelo e Limitações & \texttt{src/executar\_modelagem.py} & Wilcoxon ($p < 0{,}05$), Oráculo e P@10 CBF \\ \bottomrule
\end{tabular}%
}
\end{table}
